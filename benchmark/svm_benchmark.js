...
const CONTRACT_ADDRESS = new PublicKey("3FeFUn3mng4psjHTwWwQUZPvisxVVxZ3UVPAtVBfKG6b");
const PRIVATE_KEY = `${require("os").homedir()}/.config/solana/id.json`;  // Keypair path
const N_DOMAINS = 100;
const UNIQUE_PREFIX = 'nameprefix';

const CONTRACT_ABI = JSON.parse(fs.readFileSync("eclipse_svm.json", "utf8"));
const WALLET = Keypair.fromSecretKey(new Uint8Array(JSON.parse(fs.readFileSync(PRIVATE_KEY, "utf8"))));
